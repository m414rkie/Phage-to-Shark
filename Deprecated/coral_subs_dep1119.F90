...
yloop:	do y = 1, grid, 1
		...
		! x
		if (y .gt. 1) then
			bactcoral = bactcoral + real(bacteria(2*x,2*y)%totalpop+bacteria(2*x,2*y-1)%totalpop,8)
			lyscoral = lyscoral + real(lys(2*x,2*y)%totalpop+lys(2*x,2*y-1)%totalpop,8)
		end if

		! y
		if (x .gt. 1) then
			bactcoral = bactcoral + real(bacteria(2*x,2*y)%totalpop+bacteria(2*x-1,2*y)%totalpop,8)
			lyscoral = lyscoral + real(lys(2*x,2*y)%totalpop+lys(2*x-1,2*y)%totalpop,8)
		end if
		...
		if (x .lt. grid) then
			...
			lyscoral = lyscoral + real(lys(2*x+1,2*y)%totalpop+lys(2*x+1,2*y-1)%totalpop,8)
		end if

		! x
		if (y .lt. grid) then
			bactcoral = bactcoral + real(bacteria(2*x,2*y+1)%totalpop+bacteria(2*x-1,2*y+1)%totalpop,8)
			lyscoral = lyscoral + real(lys(2*x,2*y+1)%totalpop+bacteria(2*x-1,2*y+1)%totalpop,8)
		end if

		! normalize to area of region covered
		bactcoral = bactcoral + lyscoral
		...
		if (ran .lt. 0.5) then
			growpercent = 0.05 + 0.025*growpercent
		else
			growpercent = 0.05 - 0.025*growpercent
		end if

		grow = 1.0 + deconst*growpercent*(1.0-bacteff)
...
write(*,*) "Coral Modifiers", grow, bacteff
...
! Initialize the decay constant to primal value
...
yloop:	do y = 1, grid, 1
		...
		if (algcount .gt. 8) then
			write(*,*) "ERROR IN ALGAE COUNTING"
			stop
		end if
...
write(*,*) "Algae Modifiers: ", decay_loc, decay_loc_ini, algcount

! Find average of decay
cur_perc = percentcor(grid)
decavg = decavg/(grid*grid*cur_perc)
